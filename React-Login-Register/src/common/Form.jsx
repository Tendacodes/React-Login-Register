...
export const Form = (formitem ,buttons) => {
    ...
    return (
        <form className="flex flex-col gap-5 max-w-[400px] mx-auto border p-5 mt-12 rounded-md" action="">
            {
                formitem.map(item => <FormInput handleInputChange={handleInputChange} name={item.name} label={item.label} type={item.type} placeholder={item.placeholder} />)
            }
